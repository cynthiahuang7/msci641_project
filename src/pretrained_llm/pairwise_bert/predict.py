...
import pretrained_llm.pairwise_bert.model as mutils
...
def cli_main():
    ...
    import pretrained_llm.bert.model_bert_no_training
    parser = pretrained_llm.bert.model_bert_no_training.Classifier.add_model_specific_args(parser)

    args = parser.parse_args()

    if args.model_type == 'bert-no-training':
        import pretrained_llm.bert.model_bert_no_training as model_bert
        ...
        if torch.cuda.is_available():
            ...
            model = mutils.Classifier.load_from_checkpoint(args.checkpoint_path, map_location=torch.device('cpu'))
