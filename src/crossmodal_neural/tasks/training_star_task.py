...
from crossmodal_neural.models.InputData import InputData, InputDataTest
...
class TrainingTaskStar(object):
    def __init__(self, run_testset = False):
        self.run_testset = run_testset 
    ...
    def run(
        ...
        for epoch in tqdm(range(num_epochs), desc=f"Epoch"):
            ...
            if (epoch + 1) % 1 == 0:
                ...
                if f1_value > best_value:
                    ...
                    if multiple_gpus:
                        torch.save(
                            model.module.state_dict(), f"./crossmodal_neural/models/{output_model}",
                        )
                    else:
                        torch.save(
                            model.state_dict(), f"./crossmodal_neural/models/{output_model}",
                        )
        ...
        with open(f"./crossmodal_neural/logs/{output_log}", "w") as f:
            json.dump(result_dict, f)
        #with open(f"./crossmodal_neural/logs/best_{output_log}", "w") as f:
        #    json.dump(result_dict, f)
